...
/**
 * Cross-Browser HCM Sync Profiles screen PM Test Runner
 * 
 * Command: mvn test -Dtest=CrossBrowser22_ValidateHCMSyncProfilesTabPMRunner
 * Result: Chrome + Firefox + Edge run automatically
 * Tests: HCM Sync Profiles screen functionality in Profile Manager across browsers
 * 
 * @author AI Assistant
 */
@Listeners({
...
public class CrossBrowser22_ValidateHCMSyncProfilesTabPMRunner extends CustomizeTestNGCucumberRunner {
     ...
     * 
     * @return The tag expression template for Cross-Browser HCM Sync Profiles screen PM validation
     */
    @Override
    ...
    @Test(dataProvider = "browsers", description = "Cross-Browser HCM Sync Profiles screen PM Testing")
    public void runCrossBrowserTest(String browserName, String browserVersion, String platform) {
        ...
        try {
            ...
            LOGGER.info("Running {} scenarios for HCM Sync Profiles screen PM functionality on {}", scenarios.length, browserName.toUpperCase());
            ...
            for (Object[] scenario : scenarios) {
                ...
                try {
                    ...
                    synchronized(CrossBrowser22_ValidateHCMSyncProfilesTabPMRunner.class) {
            ...
            LOGGER.info("{} browser HCM Sync Profiles screen PM testing completed", browserName.toUpperCase());
            
        } catch (Exception e) {
            LOGGER.error("{} browser HCM Sync Profiles screen PM test failed: {}", browserName.toUpperCase(), e.getMessage());
        } finally {
