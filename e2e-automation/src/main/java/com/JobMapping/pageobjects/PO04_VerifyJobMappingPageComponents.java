	...
	public void enter_job_name_substring_in_search_bar() {
		...
		try {
			...
			if (!foundResults) {
				...
				LOGGER.warn("⚠️ All search substrings exhausted without finding results");
				LOGGER.warn("   Proceeding with last substring: '" + selectedSubstring + "'");
				ExtentCucumberAdapter.addTestStepLog("⚠️ No search substring returned results. Using: '" + selectedSubstring + "'");
			}
