	...
	public void user_should_search_for_job_profile_with_search_a_different_profile_button_on_mapped_success_profile() {
		try {
			...
			if (mapSP == false) {
				...
				ExtentCucumberAdapter.addTestStepLog("ℹ This feature requires Manually Mapped jobs to test. Please map jobs manually first.");
				ExtentCucumberAdapter.addTestStepLog("ℹ Scenario marked as SKIPPED");
			...
			if (mapSP == false) {
				...
				LOGGER.warn("⚠ Marking scenario as SKIPPED in TestNG reports");
				ExtentCucumberAdapter.addTestStepLog("⚠ SKIPPING SCENARIO: No Manually Mapped profiles available");
				ExtentCucumberAdapter.addTestStepLog("ℹ Scenario marked as SKIPPED - requires Manually Mapped jobs");
