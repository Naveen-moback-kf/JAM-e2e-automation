...
public class CrossBrowserDriverManager {
	...
	@BeforeTest
	public void launchBrowser(@Optional("chrome") String browserName, @Optional("latest") String browserVersion,
	...
	public static void initializeBrowser(String browserName, String browserVersion, String platform) {
		try {
			...
			LOGGER.info("CROSS-BROWSER INIT - Starting {} browser on {} platform", browserName.toUpperCase(), platform);
			...
			LOGGER.info("{} browser launched successfully for thread: {}", browserName.toUpperCase(),
	...
	private static WebDriver createEdgeDriver(boolean isHeadless) {
		...
		if (systemEdgeDriver != null && !systemEdgeDriver.isEmpty()) {
			...
			// Strategy 2: Try various WebDriverManager approaches with enhanced error
			// handling
				...
				if (checkNetworkConnectivity()) {
					...
					try {
						...
						try {
							...
							WebDriverManager.edgedriver().clearDriverCache().avoidFallback().setup();
							driverSetupSuccessful = true;
			...
			if (!driverSetupSuccessful) {
				try {
					...
					WebDriverManager.edgedriver().avoidBrowserDetection().avoidFallback().cachePath(cacheDir).setup();
					driverSetupSuccessful = true;
			...
			if (!driverSetupSuccessful) {
				try {
					...
					String[] possiblePaths = {
							...
							userHome + "\\Desktop\\msedgedriver.exe", userHome + "\\Desktop\\MicrosoftWebDriver.exe",
							// Project directory
							currentDir + "\\drivers\\msedgedriver.exe", currentDir + "\\msedgedriver.exe",
							// System Edge installation paths
							"C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedgedriver.exe",
							"C:\\Program Files\\Microsoft\\Edge\\Application\\msedgedriver.exe",
							// Alternative download locations
							"C:\\drivers\\msedgedriver.exe", "C:\\selenium\\msedgedriver.exe" };
			...
			if (!driverSetupSuccessful) {
				...
				LOGGER.error(
						"1. MANUAL DOWNLOAD: https://developer.microsoft.com/en-us/microsoft-edge/tools/webdriver/");
				...
				throw new RuntimeException(
						"Edge driver setup failed. Network connectivity issue detected - see log for solutions.");
			}
		}
	...
	private static boolean checkNetworkConnectivity() {
		try {
			...
			LOGGER.debug("Network connectivity check: {} (HTTP {})", isConnected ? "SUCCESS" : "FAILED", responseCode);
			return isConnected;
		...
		try {
			...
			if (driver != null) {
				...
				LOGGER.info("Closing {} browser for thread: {}",
						browserName != null ? browserName.toUpperCase() : "UNKNOWN", Thread.currentThread().getName());
	...
	public static String getBrowserInfo() {
		...
		return String.format("%s Browser (Thread: %s)", browserName.toUpperCase(), Thread.currentThread().getName());
	}
