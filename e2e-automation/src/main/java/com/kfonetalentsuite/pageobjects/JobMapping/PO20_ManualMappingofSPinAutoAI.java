	...
	public void verify_profile_with_no_bic_mapping_is_displaying_on_top_after_sorting() throws InterruptedException {
		try {
			...
			if (text.contains("Publish")) {
				...
				// Extract job details
				WebElement jobName = wait.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//tbody//tr[1]//td[2]//div[contains(text(),'(')]")));
				String jobname1 = jobName.getText();

				PO21_MapDifferentSPtoProfileInAutoAI.orgJobName.set(jobname1.split("-", 2)[0].trim());
				PO21_MapDifferentSPtoProfileInAutoAI.orgJobCode
						.set(jobname1.split("-", 2)[1].trim().substring(1, jobname1.split("-", 2)[1].length() - 2));

				PageObjectHelper.log(LOGGER,
						"Organization Job name / Job Code of Profile with No BIC Profile Mapped : "
								+ PO21_MapDifferentSPtoProfileInAutoAI.orgJobName.get() + "/"
								+ PO21_MapDifferentSPtoProfileInAutoAI.orgJobCode.get());
				...
				PO21_MapDifferentSPtoProfileInAutoAI.orgJobDepartment
						.set((deptText.equals("-") || deptText.isEmpty() || deptText.isBlank()) ? "NULL" : deptText);
				...
				PO21_MapDifferentSPtoProfileInAutoAI.orgJobFunction
						.set((funcText.equals("-") || funcText.isEmpty() || funcText.isBlank()) ? "NULL" : funcText);
			}
		} catch (NoSuchElementException e) {
			PO21_MapDifferentSPtoProfileInAutoAI.mapSP.set(false);
			PageObjectHelper.log(LOGGER, "Currently, All Profiles in Job Mapping are Mapped with BIC Profiles");
			PageObjectHelper.handleError(LOGGER,
					"verify_profile_with_no_bic_mapping_is_displaying_on_top_after_sorting",
	...
	public void user_should_verify_search_a_different_profile_button_is_displaying_on_manually_mapped_success_profile() {
		if (PO21_MapDifferentSPtoProfileInAutoAI.manualMapping.get()) {
			try {
				...
				PageObjectHelper.log(LOGGER,
						"Search a Different Profile is Displaying on matched success profile which is on the Top of the Profiles List");
			} catch (Exception e) {
				PageObjectHelper.handleError(LOGGER,
						"user_should_verify_search_a_different_profile_button_is_displaying_on_manually_mapped_success_profile",
	...
	public void click_on_manually_mapped_profile_name_of_job_profile_on_top_of_profiles_list() {
		if (PO21_MapDifferentSPtoProfileInAutoAI.manualMapping.get()) {
			try {
				...
				PageObjectHelper.log(LOGGER,
						"Clicked on Manually Mapped Profile with name " + MappedProfileNameText
				...
				PageObjectHelper.handleError(LOGGER,
						"click_on_manually_mapped_profile_name_of_job_profile_on_top_of_profiles_list",
