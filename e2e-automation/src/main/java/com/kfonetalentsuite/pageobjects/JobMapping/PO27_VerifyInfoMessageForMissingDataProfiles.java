...
public class PO27_VerifyInfoMessageForMissingDataProfiles extends DriverManager {
	...
	private static final Logger LOGGER = (Logger) LogManager
			.getLogger(PO27_VerifyInfoMessageForMissingDataProfiles.class);
	...
	// Fields for dynamic row-based approach (used by old methods - kept for
	...
	// Job details for the first profile with info message (STATIC to persist across
	...
	// Job details for the second profile with info message (STATIC to persist
	// across page object instances)
		...
		try {
			...
			while (scrollCount < maxScrolls) {
				...
				if (currentProfiles >= targetProfiles) {
					LOGGER.info("SUCCESS: Target reached: {} profiles loaded (target: {})", currentProfiles,
							targetProfiles);
					break;
				}
	...
	/**
	 * ENHANCED: Extract job details directly from info message element (more
	 * reliable after scrolling)
	 * 
	...
	private void extractJobDetailsDirectlyFromInfoMessage(WebElement infoMessageElement, boolean isSecondProfile)
			throws IOException {
		try {
			LOGGER.info("INFO: Extracting job details directly from info message element (second profile: {})",
					isSecondProfile);
			...
			LOGGER.info("STATUS: Direct extraction: Info message row {}, job details row {}", infoRowIndex,
					jobDetailsRowIndex);
	...
	/**
	 * ENHANCED: Efficiently check current visible profiles for AutoMapped ones with
	 * info messages Returns immediately upon finding the first AutoMapped profile
	 ...
	 * @return true if AutoMapped profile found and details extracted, false
	 *         otherwise
	 */
	@SuppressWarnings("unused")
	private boolean checkCurrentProfilesForAutoMapped(List<WebElement> infoMessages, int searchAttempt) {
		LOGGER.info("INFO: Checking {} visible profiles for AutoMapped ones (search attempt {})", infoMessages.size(),
				searchAttempt);
				...
				try {
					...
					if (rowIndex > 0) {
						...
						if (!isAutoMappedProfile(rowIndex)) {
							...
							LOGGER.info("SKIP: Skipping Profile {} (row {}) - Manual Mapping profile", profileNumber,
									rowIndex);
						...
						try {
							...
							if (jobNameWithInfoMessage.isEmpty() && jobCodeWithInfoMessage.isEmpty()) {
								LOGGER.warn(
										"WARNING: First extraction attempt failed for first profile at row {} - trying alternative approach",
										rowIndex);
							...
							if (jobNameWithInfoMessage.isEmpty() && jobCodeWithInfoMessage.isEmpty()) {
								throw new IOException(
										"Failed to extract job details for first profile even with alternative methods - row "
												+ rowIndex);
							}

						} catch (Exception extractionError) {
							LOGGER.error("ERROR: Failed to extract job details for first profile at row {}: {}",
									rowIndex, extractionError.getMessage());

							// Try one more time with the direct approach
							try {
								LOGGER.info(
										"INFO: Attempting direct extraction from info message element for first profile...");
								extractJobDetailsDirectlyFromInfoMessage(infoMessage, false);
								...
								LOGGER.error("ERROR: All extraction attempts failed for first profile: {}",
										finalError.getMessage());
								throw new IOException(
										"Failed to extract first profile details after multiple attempts: "
												+ finalError.getMessage());
							}
						}
						...
						LOGGER.info("SUCCESS: AutoMapped Profile {} found with Info Message (table row {})",
								profileNumber, rowIndex);
	 ...
	 * AutoMapped profiles have "View Other Matches" buttons while Manual Mapping
	 * profiles have "Search a different profile" buttons
	 * 
	...
	private boolean isAutoMappedProfile(int rowIndex) {
		try {
			// Check for "View Other Matches" button (AutoMapped profiles) - in RIGHT table
			// (kf-job-container)
			...
			// Check for "Search a different profile" button (Manual Mapping profiles) - in
			// RIGHT table (kf-job-container)
	...
	public void find_and_verify_profile_with_missing_data_has_info_message_displayed() throws IOException {
		try {
			...
			// If no info messages found in visible content, try scrolling to find more
			// profiles
			if (infoMessages.isEmpty()) {
				...
				if (infoMessages.isEmpty()) {
					...
					for (String xpath : alternativeXPaths) {
						...
						if (!infoMessages.isEmpty()) {
							LOGGER.info("STATUS: Found {} info messages using alternative locator",
									infoMessages.size());
							break;
						}
					}
	...
	public void find_profile_with_missing_data_and_info_message() throws IOException {
		...
		try {
			...
			Set<String> processedJobCodes = new HashSet<>(); // Track which jobs we've already checked (for virtual
																// scrolling)
			...
			while (scrollAttempts < maxScrollAttempts) {
				...
				// Get current visible job rows (each profile has 3 rows: job data + function +
				// info/separator)
				// Note: Table uses VIRTUAL SCROLLING - only ~30 rows in DOM, so process ALL
				// rows each time
				List<WebElement> currentRows = driver
				...
				List<WebElement> rightTableAllRows = driver
						.findElements(By.xpath("//div[@id='kf-job-container']//tbody//tr"));
				// Pre-fetch all row classes using JavaScript (instant, no round-trips)
				JavascriptExecutor js = (JavascriptExecutor) driver;
				@SuppressWarnings("unchecked")
				List<String> rightTableRowClasses = (List<String>) js
						.executeScript("var rows = document.querySelectorAll('#kf-job-container tbody tr');"
								+ "return Array.from(rows).map(r => r.className || '');");

				// Process ALL visible rows (virtual scrolling means indices reset, so can't use
				...
				for (int i = 0; i < currentRows.size() - 2; i += 3) {
					try {
						...
						// Extract job code to check if we've already processed this job (virtual
						// scrolling)
						String jobCode = "";
						try {
						...
						// STEP 2: Check RIGHT table for "View Other Matches" button using
						// SEPARATOR-BASED BOUNDARIES
						...
						// Use cached RIGHT table rows and classes (fetched once per scroll, not per
						// profile)
						try {
							...
							if (rightProfileStartRow >= 0 && rightProfileEndRow >= rightProfileStartRow) {
								...
								if (dataRows.size() == 2) {
									...
									// If no button found in 2nd row = Manual Mapping (has "Search a different
									// profile")
								}
						...
						// Extract Function/Sub-function from the function row (after "Function /
						// Sub-function: " text)
						...
						if (hasMissingData) {
							...
							// Store row index for button click (RIGHT table row number where button was
							// found)
				...
				if (!currentRows.isEmpty()) {
					...
					jsExecutor.executeScript("var container = document.getElementById('org-job-container');"
				...
				List<WebElement> rowsAfterScroll = driver
						.findElements(By.xpath("//div[@id='org-job-container']//tbody//tr"));
			...
			throw new SkipException(
					"SKIPPED: No AutoMapped profiles with missing data and info message found after checking all "
							+ profilesChecked
	...
	public void find_second_profile_with_missing_data_and_info_message() throws IOException {
		...
		try {
			...
			while (scrollAttempts < maxScrollAttempts) {
				...
				@SuppressWarnings("unchecked")
				List<String> rightTableRowClasses = (List<String>) js2
						.executeScript("var rows = document.querySelectorAll('#kf-job-container tbody tr');"
								+ "return Array.from(rows).map(r => r.className || '');");
				...
				for (int i = 0; i < currentRows.size() - 2; i += 3) {
					try {
						...
						String jobCode = "";
						// PERFORMANCE: Use getAttribute('textContent') instead of getText() (much
						// faster)
						String jobName = "Unknown";
						try {
							List<WebElement> jobNameElements = jobDataRow.findElements(By.xpath(".//td[2]//div"));
							if (!jobNameElements.isEmpty()) {
								// Use textContent attribute (instant) instead of getText() (slow - waits for
								// visibility)
								String jobNameCodeText = jobNameElements.get(0).getAttribute("textContent");
								if (jobNameCodeText != null) {
									jobNameCodeText = jobNameCodeText.trim();
						...
						try {
							...
							LOGGER.debug("Error checking RIGHT table boundaries for second profile: {}",
									e.getMessage());
						}
						...
						// Profile has info message AND is AutoMapped - extract details
						// PERFORMANCE: Use getAttribute('textContent') instead of getText() for speed
						// Note: jobName already extracted earlier in the loop for firstJobName
						// comparison
						...
						if (hasMissingData) {
							LOGGER.info("✓ Found SECOND AutoMapped profile: {} (Profile #{})", jobName,
									profilesChecked);
							LOGGER.info("  Details - Grade: {}, Dept: {}, Func: {}", grade, department,
									functionSubfunction);
			...
			ExtentCucumberAdapter.addTestStepLog(
					" No SECOND AutoMapped profile found with missing data and info message (checked all "
							+ profilesChecked + " profiles)");
			LOGGER.warn(
					"SKIPPING SCENARIO: No SECOND AutoMapped profile with missing data and info message found (checked all "
							+ profilesChecked + " profiles)");

			// Throw SkipException to mark scenario as SKIPPED in TestNG (not FAILED)
			throw new SkipException(
					"SKIPPED: No SECOND AutoMapped profile with missing data and info message found after checking all "
							+ profilesChecked
	...
	public void extract_job_details_from_second_profile_with_info_message() throws IOException {
		// NOTE: Job details are already extracted and logged during
		// find_second_profile_with_missing_data_and_info_message()
		...
		try {
			if (secondJobNameWithInfoMessage.isEmpty() && secondJobCodeWithInfoMessage.isEmpty()) {
				throw new IOException(
						"Second profile job details not found. Please call find_second_profile_with_missing_data_and_info_message() first.");
			}
	...
	public void verify_info_message_contains_text_about_reduced_match_accuracy_due_to_missing_data()
		...
		try {
			...
			PageObjectHelper.handleError(LOGGER,
					"verify_info_message_contains_text_about_reduced_match_accuracy_due_to_missing_data",
	...
	public void find_and_verify_second_profile_with_missing_data_has_info_message_displayed() throws IOException {
		try {
			...
			if (infoMessages.isEmpty()) {
				...
				infoMessages = driver.findElements(By.xpath(
						"//div[@id='org-job-container']//div[@role='button' and @aria-label='Reduced match accuracy due to missing data']"));
				LOGGER.info("STATUS: After scrolling: Found {} info messages for second profile verification",
						infoMessages.size());
			}
			...
			throw new IOException(
					"Failed to find and verify second profile with missing data has Info Message: " + e.getMessage());
		}
	}

	public void extract_job_details_from_profile_with_info_message() throws IOException {
		// NOTE: Job details are already extracted and logged during
		// find_profile_with_missing_data_and_info_message()
		// This method validates that extraction was successful
		try {
			if (jobNameWithInfoMessage.isEmpty() && jobCodeWithInfoMessage.isEmpty()) {
				throw new IOException(
						"First profile job details not found. Please call find_profile_with_missing_data_and_info_message() first.");
			}
			// Details already logged during search - no need to log again
		} catch (Exception e) {
	...
	public void verify_info_message_is_still_displayed_in_job_comparison_page() throws IOException {
		try {
			...
			Assert.assertFalse(infoMessages.isEmpty(),
					"Info Message should still be displayed in Job Comparison page for first profile");
	...
	public void verify_info_message_is_still_displayed_in_job_comparison_page_for_second_profile() throws IOException {
		try {
			...
			Assert.assertFalse(infoMessages.isEmpty(),
					"Info Message should still be displayed in Job Comparison page for second profile");
			...
			throw new IOException(
					"Failed to verify Info Message persistence in Job Comparison page for second profile: "
							+ e.getMessage());
		}
	}

	public void verify_info_message_contains_text_about_reduced_match_accuracy_due_to_missing_data_for_second_profile()
			throws IOException {
		try {
			...
			Assert.assertFalse(infoMessageTexts.isEmpty(),
					"Info Message text elements not found for second profile validation");

			boolean correctTextFound = false;
			WebElement targetMessageText = null;

			// If multiple info messages exist, try to use the second one for second profile
			// validation
			...
			throw new IOException("Failed to verify second profile Info Message text for second profile validation: "
					+ e.getMessage());
		}
	}
	...
	public void click_on_button_for_profile_with_info_message(String buttonText) throws IOException {
		try {
			...
			if (currentRowIndex <= 0) {
				throw new IOException(
						"No valid profile row index found. Call find_profile_with_missing_data_and_info_message() first.");
			}

			// Check this is an AutoMapped profile before clicking
			if (!isAutoMappedProfile(currentRowIndex)) {
				throw new IOException("Profile at row " + currentRowIndex
						+ " is not an AutoMapped profile - cannot click '" + buttonText + "' button.");
			}
			...
			// Find "View Other Matches" button for AutoMapped profiles - in RIGHT table
			// (kf-job-container)
			String[] viewMatchesButtonXPaths = {
					String.format("//div[@id='kf-job-container']//tbody//tr[%d]//button[@id='view-matches']",
							currentRowIndex),
					String.format(
							"//div[@id='kf-job-container']//tbody//tr[%d]//button[contains(@aria-label, 'View other matches')]",
							currentRowIndex),
			...
			if (targetButton == null) {
				throw new IOException(
						"Could not find '" + buttonText + "' button for AutoMapped profile at row " + currentRowIndex);
			}
	...
	public void click_on_button_for_second_profile_with_info_message(String buttonText) throws IOException {
		try {
			...
			if (secondCurrentRowIndex <= 0) {
				throw new IOException(
						"No valid second profile row index found. Call find_second_profile_with_missing_data_and_info_message() first.");
			}

			// Check this is an AutoMapped profile before clicking
			if (!isAutoMappedProfile(secondCurrentRowIndex)) {
				throw new IOException("Second profile at row " + secondCurrentRowIndex
						+ " is not an AutoMapped profile - cannot click '" + buttonText + "' button.");
			}
			...
			// Find "View Other Matches" button for AutoMapped second profile - in RIGHT
			// table (kf-job-container)
			String[] viewMatchesButtonXPaths = {
					String.format("//div[@id='kf-job-container']//tbody//tr[%d]//button[@id='view-matches']",
							secondCurrentRowIndex),
					String.format(
							"//div[@id='kf-job-container']//tbody//tr[%d]//button[contains(@aria-label, 'View other matches')]",
							secondCurrentRowIndex),
					String.format(
			...
			for (String xpath : viewMatchesButtonXPaths) {
				try {
					...
					for (WebElement button : buttons) {
						if (button.isDisplayed()) {
							targetButton = button;
							LOGGER.debug("Found '{}' button for second profile at row {}", buttonText,
									secondCurrentRowIndex);
							break;
						}
					}
			...
			if (targetButton == null) {
				throw new IOException("Could not find '" + buttonText + "' button for second AutoMapped profile at row "
						+ secondCurrentRowIndex);
			}
	...
	public void verify_user_is_navigated_to_job_comparison_page() throws IOException {
		try {
			...
			WebElement compareAndSelectHeader = wait
					.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//h1[@id='compare-desc']")));
	...
	public void extract_job_details_from_job_comparison_page() throws IOException {
		try {
			...
			// Extract job details using text-based approach from the Organization Job
			// section
	...
	public void extract_job_details_from_job_comparison_page_for_second_profile() throws IOException {
		try {
			LOGGER.info(
					"Extracting job details from Job Comparison page (Organization Job section) for second profile");
			safeSleep(2000);
			...
			LOGGER.info(
					"Extracted Job Details from Job Comparison page (Organization Job section) for second profile:");
			LOGGER.info("  Job Name: {}", comparisonJobName);
			LOGGER.info("  Job Code: {}", comparisonJobCode);
			LOGGER.info("  Grade: {}", comparisonGrade);
			LOGGER.info("  Department: {}", comparisonDepartment);
			...
			String normalizedComparisonGrade = comparisonGrade.equals("-") ? "" : comparisonGrade;
			String normalizedStoredDept = secondDepartmentWithInfoMessage.equals("-") ? ""
					: secondDepartmentWithInfoMessage;
			String normalizedComparisonDept = comparisonDepartment.equals("-") ? "" : comparisonDepartment;
	...
	public void verify_job_details_match_between_job_mapping_and_job_comparison_pages() throws IOException {
		// NOTE: Verification is now done inline within
		// extract_job_details_from_job_comparison_page()
		// This method is kept for backward compatibility with step definitions - no
	...
	public void verify_job_details_match_between_job_mapping_and_job_comparison_pages_for_second_profile()
			throws IOException {
		// NOTE: Verification is now done inline within
		// extract_job_details_from_job_comparison_page_for_second_profile()
		// This method is kept for backward compatibility with step definitions - no
		// additional action needed
	}
	...
	public void verify_info_message_contains_same_text_about_reduced_match_accuracy() throws IOException {
		try {
			LOGGER.info(
					"Verifying Info Message contains same text about reduced match accuracy in Job Comparison page");
			safeSleep(2000);
	...
	public void verify_info_message_contains_same_text_about_reduced_match_accuracy_for_second_profile()
			throws IOException {
		try {
			LOGGER.info(
					"Verifying Info Message contains same text about reduced match accuracy for second profile in Job Comparison page");
			safeSleep(2000);
			...
			Assert.assertFalse(infoMessageTexts.isEmpty(),
					"Info Message text not found in Job Comparison page for second profile");
			...
			for (WebElement messageText : infoMessageTexts) {
				if (messageText.isDisplayed()) {
					...
					if (actualText.contains("Reduced match accuracy due to missing data")) {
						correctTextFound = true;
						LOGGER.info("Found correct Info Message text in Job Comparison page for second profile: {}",
								actualText);
						break;
					}
				}
			...
			throw new IOException(
					"Failed to verify Info Message text consistency in Job Comparison page for second profile: "
							+ e.getMessage());
		}
	}
