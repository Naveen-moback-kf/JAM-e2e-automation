...
public class PO40_ValidateSelectAndPublishAllJobProfilesinJAM {
	...
	// Determine if scrolling was limited by maxScrollAttempts
		...
		if (reachedScrollLimit) {
			...
			if (totalProfiles > 0) {
				LOGGER.warn(" Total profiles in system: " + totalProfiles + ", but only " + currentRowCount + " loaded");
				LOGGER.warn(" Validation will be based on loaded profiles only (ignoring " + (totalProfiles - currentRowCount) + " unloaded profiles)");
			}
			ExtentCucumberAdapter.addTestStepLog(" Max scroll limit reached. Validating " + currentRowCount + " loaded profiles only");
		} else {
		...
		if (reachedScrollLimit && totalProfiles > 0) {
			...
			LOGGER.info(" All calculations and validations will be based on " + currentRowCount + " loaded profiles only");
		}
	...
	public void verify_published_profiles_count_matches_in_view_published_screen() {
		try {
			...
			if (publishedProfilesCountAfter == totalPublishedCount) {
				...
				LOGGER.warn(" Count mismatch! Expected: " + totalPublishedCount + ", Actual: " + publishedProfilesCountAfter);
	...
	public void monitor_and_validate_progressive_batch_publishing_until_completion() throws InterruptedException {
		...
		try {
			...
			if (remainingToPublishFinal > 0) {
				...
				LOGGER.warn(" Publishing not completed within " + maxWaitMinutes + " minutes (" + totalChecks + " checks at " + REFRESH_INTERVAL_SECONDS + "s intervals)");
				...
				ExtentCucumberAdapter.addTestStepLog(" Publishing timeout after " + totalChecks + " checks. Still remaining: " + remainingToPublishFinal + " profiles");
			}
