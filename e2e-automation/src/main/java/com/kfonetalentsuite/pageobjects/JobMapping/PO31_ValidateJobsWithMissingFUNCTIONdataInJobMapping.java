...
public class PO31_ValidateJobsWithMissingFUNCTIONdataInJobMapping extends BasePageObject {

	private static final Logger LOGGER = LogManager.getLogger(PO31_ValidateJobsWithMissingFUNCTIONdataInJobMapping.class);
	...
	public void find_job_in_jobs_missing_data_screen_where_function_is_na() throws IOException {
		try {
			...
			PageObjectHelper.log(LOGGER, " REVERSE SCENARIO (Scenario 2): Finding job with missing Function data...");
			...
			if (preferredJobRow != null) {
				...
				PageObjectHelper.log(LOGGER, "Found job profile where Function is missing - proceeding with validation");
				...
				if (cells.size() >= 4) {
					...
					PageObjectHelper.log(LOGGER, "   Function/Subfunction: " + functionSubfunction + " (Missing)");
	...
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_function_data()
		...
		try {
			...
			PageObjectHelper.log(LOGGER, "Info Message verified for profile with missing Function data");
	...
	public void find_job_profile_in_job_mapping_page_where_function_is_missing() throws IOException {
		try {
			...
			while (scrollAttempt <= maxScrollAttempts && !matchFound) {
				...
				for (int i = lastCheckedIndex; i < infoMessageRows.size(); i++) {
					...
					try {
						...
						if (functionIsMissing) {
														PageObjectHelper.log(LOGGER, "Found suitable profile: " + jobName + " - Function missing");
			...
			if (!matchFound) {
				...
				PageObjectHelper.log(LOGGER, " No profiles found with missing Function");
