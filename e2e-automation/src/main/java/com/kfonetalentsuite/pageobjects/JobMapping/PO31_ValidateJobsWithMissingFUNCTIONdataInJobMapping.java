...
public class PO31_ValidateJobsWithMissingFUNCTIONdataInJobMapping extends DriverManager {
	...
	public void find_job_in_jobs_missing_data_screen_where_function_is_na() throws IOException {
		try {
			...
			LOGGER.info(
					"REVERSE SCENARIO (Scenario 2): Forward scenario found profile - will search for jobs with missing Function");
			LOGGER.info("Looking for: Function missing (ignoring Grade & Department data)");
			ExtentCucumberAdapter
					.addTestStepLog(" REVERSE SCENARIO (Scenario 2): Finding job with missing Function data...");
			...
			for (int i = 0; i < allJobRows.size(); i++) {
				...
				if (cells.size() >= 4) {
					...
					boolean functionIsMissing = !hasFunctionSubfunctionData(functionSubfunction, "Job Mapping",
							jobName);
			...
			if (preferredJobRow != null) {
				...
				ExtentCucumberAdapter
						.addTestStepLog("... Found job profile where Function is missing - proceeding with validation");
				...
				if (cells.size() >= 4) {
					...
					ExtentCucumberAdapter
							.addTestStepLog("   Function/Subfunction: " + functionSubfunction + " (Missing)");
	 ...
	 */
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_function_data()
			throws IOException {
		try {
			String fullJobName = jobDetailsFromMissingDataScreen.get("jobName");
			...
			Assert.assertTrue(infoMessageFound,
					"Info message not found on searched profile indicating missing Function data");
	...
	/**
	 * Find job profile with missing FUNCTION in Job Mapping page - FORWARD SCENARIO
	 * (Scenario 1): - Used in Forward flow: Find job on Job Mapping page with
	 * missing Function + info message - Searches Job Mapping screen for FIRST job
	 * with missing Function and info message - Required conditions: Function
	 * missing + Info message displayed (ignoring Grade/Department data)
	 */
	public void find_job_profile_in_job_mapping_page_where_function_is_missing() throws IOException {
		try {
			...
			LOGGER.info(
					"FORWARD SCENARIO (Scenario 1): Searching for first job with info message where Function is missing");
			LOGGER.info("Looking for: info message + Function missing (ignoring Grade & Department data)");
			ExtentCucumberAdapter.addTestStepLog(
					" FORWARD SCENARIO (Scenario 1): Finding FIRST job with info message and missing Function...");
			...
			while (scrollAttempt <= maxScrollAttempts && !matchFound) {
				...
				for (int i = lastCheckedIndex; i < infoMessageRows.size(); i++) {
					...
					try {
						...
						// approach (ULTRA-OPTIMIZED)
						try {
							List<WebElement> jobNameElements = jobDataRow
									.findElements(By.xpath(".//td[2]//div | .//td[position()=2]//div"));
							if (!jobNameElements.isEmpty()) {
								String jobNameCodeText = ((String) js.executeScript("return arguments[0].textContent;",
										jobNameElements.get(0))).trim();
						...
						try {
							...
							if (!gradeElements.isEmpty()) {
								grade = ((String) js.executeScript("return arguments[0].textContent;",
										gradeElements.get(0))).trim();
							}
						} catch (Exception e) {
						...
						try {
							...
							if (!departmentElements.isEmpty()) {
								department = ((String) js.executeScript("return arguments[0].textContent;",
										departmentElements.get(0))).trim();
							}
						} catch (Exception e) {
						...
						boolean functionIsMissing = !hasFunctionSubfunctionData(functionSubfunction,
								"Info Message Profile", jobName);

						LOGGER.info("Profile " + profileNumber + " - Function Missing: " + functionIsMissing
								+ " (Required: TRUE)");

						// If match found, store and STOP
						if (functionIsMissing) {
							LOGGER.info("... MATCH FOUND in batch " + scrollAttempt + "! Profile " + profileNumber
									+ " has missing Function");
							LOGGER.info("... Selected: " + jobName + " (Code: " + jobCode + ")");
							ExtentCucumberAdapter
									.addTestStepLog("... Found suitable profile: " + jobName + " - Function missing");
				...
				if (scrollAttempt < maxScrollAttempts) {
					...
					// Log BEFORE scroll
					int rowsBeforeScroll = driver.findElements(By.xpath("//div[@id='org-job-container']//tbody//tr"))
							.size();
					int infoMsgsBeforeScroll = driver.findElements(By.xpath(
							"//div[@id='org-job-container']//tr[.//div[@role='button' and contains(@aria-label, 'Reduced match accuracy')]]"))
							.size();
					LOGGER.debug("BEFORE scroll - Total rows: {}, Info messages: {}", rowsBeforeScroll,
							infoMsgsBeforeScroll);
					...
					int rowsAfterScroll = driver.findElements(By.xpath("//div[@id='org-job-container']//tbody//tr"))
							.size();
					int infoMsgsAfterScroll = driver.findElements(By.xpath(
							"//div[@id='org-job-container']//tr[.//div[@role='button' and contains(@aria-label, 'Reduced match accuracy')]]"))
							.size();
					LOGGER.debug("AFTER scroll - Total rows: {}, Info messages: {}", rowsAfterScroll,
							infoMsgsAfterScroll);
					LOGGER.debug("Change: Rows +{}, Info messages +{}", (rowsAfterScroll - rowsBeforeScroll),
							(infoMsgsAfterScroll - infoMsgsBeforeScroll));
			...
			if (!matchFound) {
				LOGGER.info(" No profiles with missing Function found (checked " + lastCheckedIndex
						+ " profiles across " + scrollAttempt + " batches)");
				ExtentCucumberAdapter.addTestStepLog(" No profiles found with missing Function");

				String skipMsg = "SKIPPING SCENARIO: No profiles found with missing Function (checked "
