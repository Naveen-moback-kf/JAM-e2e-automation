...
public class PO31_ValidateJobsWithMissingFUNCTIONdataInJobMapping extends DriverManager {
	...
	public void find_job_in_jobs_missing_data_screen_where_function_is_na() throws IOException {
		try {
			...
			ExtentCucumberAdapter.addTestStepLog(" REVERSE SCENARIO (Scenario 2): Finding job with missing Function data...");
		...
		if (preferredJobRow != null) {
			...
			ExtentCucumberAdapter.addTestStepLog("... Found job profile where Function is missing - proceeding with validation");
			...
			List<WebElement> cells = preferredJobRow.findElements(By.xpath(".//td"));
				...
				if (cells.size() >= 4) {
					...
					ExtentCucumberAdapter.addTestStepLog("   Function/Subfunction: " + functionSubfunction + " (Missing)");
	...
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_function_data() throws IOException {
		try {
			...
			Assert.assertTrue(infoMessageFound, "Info message not found on searched profile indicating missing Function data");
			
			LOGGER.info("✓ Info Message verified");
			ExtentCucumberAdapter.addTestStepLog("Info Message verified for profile with missing Function data");
	...
	public void find_job_profile_in_job_mapping_page_where_function_is_missing() throws IOException {
		...
		ExtentCucumberAdapter.addTestStepLog(" FORWARD SCENARIO (Scenario 1): Finding FIRST job with info message and missing Function...");
		...
		while (scrollAttempt <= maxScrollAttempts && !matchFound) {
			...
			for (int i = lastCheckedIndex; i < infoMessageRows.size(); i++) {
				...
				try {
					...
					// Extract Job Name and Code from column 2 (NAME / JOB CODE column) - Feature 28 approach (ULTRA-OPTIMIZED)
					try {
						List<WebElement> jobNameElements = jobDataRow.findElements(
							By.xpath(".//td[2]//div | .//td[position()=2]//div")
						);
						if (!jobNameElements.isEmpty()) {
							String jobNameCodeText = ((String) js.executeScript("return arguments[0].textContent;", jobNameElements.get(0))).trim();
					...
					// Extract Grade from column 3 (GRADE column) - Feature 28 approach (ULTRA-OPTIMIZED)
					try {
						List<WebElement> gradeElements = jobDataRow.findElements(
							By.xpath(".//td[3]//div | .//td[position()=3]//div")
						);
						if (!gradeElements.isEmpty()) {
							grade = ((String) js.executeScript("return arguments[0].textContent;", gradeElements.get(0))).trim();
						}
			} catch (Exception e) {
				// Could not extract grade
			}
					
					// Extract Department from column 4 (DEPARTMENT column) - Feature 28 approach (ULTRA-OPTIMIZED)
					try {
						...
						if (!departmentElements.isEmpty()) {
							department = ((String) js.executeScript("return arguments[0].textContent;", departmentElements.get(0))).trim();
						}
			} catch (Exception e) {
				...
				if (functionIsMissing) {
					LOGGER.info("... MATCH FOUND in batch " + scrollAttempt + "! Profile " + profileNumber + " has missing Function");
					LOGGER.info("... Selected: " + jobName + " (Code: " + jobCode + ")");
					ExtentCucumberAdapter.addTestStepLog("... Found suitable profile: " + jobName + " - Function missing");
			...
			if (scrollAttempt < maxScrollAttempts) {
				...
				// Log BEFORE scroll
				int rowsBeforeScroll = driver.findElements(By.xpath("//div[@id='org-job-container']//tbody//tr")).size();
				int infoMsgsBeforeScroll = driver.findElements(By.xpath("//div[@id='org-job-container']//tr[.//div[@role='button' and contains(@aria-label, 'Reduced match accuracy')]]")).size();
				LOGGER.debug("BEFORE scroll - Total rows: {}, Info messages: {}", rowsBeforeScroll, infoMsgsBeforeScroll);
				...
				// Log AFTER scroll and waits
				int rowsAfterScroll = driver.findElements(By.xpath("//div[@id='org-job-container']//tbody//tr")).size();
				int infoMsgsAfterScroll = driver.findElements(By.xpath("//div[@id='org-job-container']//tr[.//div[@role='button' and contains(@aria-label, 'Reduced match accuracy')]]")).size();
				LOGGER.debug("AFTER scroll - Total rows: {}, Info messages: {}", rowsAfterScroll, infoMsgsAfterScroll);
				LOGGER.debug("Change: Rows +{}, Info messages +{}", (rowsAfterScroll - rowsBeforeScroll), (infoMsgsAfterScroll - infoMsgsBeforeScroll));
		...
		if (!matchFound) {
			LOGGER.info(" No profiles with missing Function found (checked " + lastCheckedIndex + " profiles across " + scrollAttempt + " batches)");
			ExtentCucumberAdapter.addTestStepLog(" No profiles found with missing Function");
