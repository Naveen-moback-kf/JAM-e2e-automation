...
public class PO36_ValidateSelectAllWithFiltersFunctionality_PM {
	...
	public void apply_filter_and_verify_profiles_count_in_hcm_sync_profiles_screen_for_feature36() {
		try {
			...
			try {
				...
				var kfGradeHeader = driver.findElements(By.xpath("//thcl-expansion-panel-header[@id='expansion-panel-header-0']"));
			...
			if (!kfGradeOptions.isEmpty()) {
				...
				try {
					...
					var levelsHeader = driver.findElements(By.xpath("//thcl-expansion-panel-header[@id='expansion-panel-header-1']"));
	...
	public void user_should_validate_all_filtered_results_match_the_applied_filter_in_hcm_sync_profiles_screen_for_feature36() {
		try {
			...
			// Get all profile name elements (same XPath as search validation for consistency)
			...
			// 1. Identify the column index for the filter type (KF Grade = column X, Levels = column Y)
	...
	public void apply_different_filter_for_alternative_validation_in_hcm_sync_profiles_screen_for_feature36() {
		...
		try {
			...
			var filterOptions = driver.findElements(By.xpath("//div[contains(@class,'filter-options') or contains(@class,'filters-container')]"));
			...
			if (filterOptions.isEmpty() || !filterOptions.get(0).isDisplayed()) {
				...
				var filtersBtn = driver.findElements(By.xpath("//button[contains(@class,'filters') or contains(text(),'Filters')]"));
		...
		// If first was KF Grade, apply KF Grade again with different value
			...
			if (!kfGradeHeader.isEmpty()) {
				// IMPORTANT: For alternative validation, force fresh rendering by toggling panel
				// Check current state
				...
				while (kfGradeOptions.isEmpty() && retryCount < maxRetries) {
					retryCount++;
					LOGGER.info("Attempt " + retryCount + "/" + maxRetries + ": Looking for KF Grade options...");
					
					// DYNAMIC XPATH: Find all available KF Grade options without hardcoding values
					// Strategy 1: Direct descendant approach
					kfGradeOptions = driver.findElements(By.xpath("//thcl-expansion-panel-header[@id='expansion-panel-header-0']/following-sibling::div//kf-checkbox"));
					...
					if (kfGradeOptions.isEmpty() && retryCount < maxRetries) {
						LOGGER.warn("No KF Grade options found on attempt " + retryCount + ", waiting 1 second before retry...");
						Thread.sleep(1000);
					}
				}
					...
					if (!kfGradeOptions.isEmpty()) {
						...
						String optionXPath = "(//thcl-expansion-panel-header[@id='expansion-panel-header-0']/following-sibling::div//kf-checkbox)[" + (optionIndex + 1) + "]/parent::*";
			...
			if (!levelsHeader.isEmpty()) {
				...
				while (levelsOptions.isEmpty() && retryCount < maxRetries) {
					...
					// DYNAMIC XPATH: Find all available Levels options without hardcoding values
					// Strategy 1: Direct descendant approach
					levelsOptions = driver.findElements(By.xpath("//thcl-expansion-panel-header[@id='expansion-panel-header-1']/following-sibling::div//kf-checkbox"));
					...
					if (levelsOptions.isEmpty() && retryCount < maxRetries) {
						LOGGER.warn("No Levels options found on attempt " + retryCount + ", waiting 1 second before retry...");
						Thread.sleep(1000);
					}
				}
					...
					if (!levelsOptions.isEmpty()) {
						...
						// Re-find the element to avoid stale reference
						// Build XPath to find the specific option by index
						String optionXPath = "(//thcl-expansion-panel-header[@id='expansion-panel-header-1']/following-sibling::div//kf-checkbox)[" + (optionIndex + 1) + "]/parent::*";
						WebElement elementToClick = driver.findElement(By.xpath(optionXPath));
			...
			for (int i = 0; i < selectedRows.size(); i++) {
				try {
					...
					try {
						...
						LOGGER.warn("⚠️ Skipped profile at row " + (rowIndex + 1) + " (could not extract title)");
						continue;
					}
	...
	public void scroll_down_to_load_all_second_filter_results_in_hcm_sync_profiles_screen_for_feature36() throws InterruptedException {
		...
		try {
			...
			int invalidCount = checkNewProfilesForInvalidSelectionsForFilter(0, initialCount, firstFilterType, firstFilterValue, 
