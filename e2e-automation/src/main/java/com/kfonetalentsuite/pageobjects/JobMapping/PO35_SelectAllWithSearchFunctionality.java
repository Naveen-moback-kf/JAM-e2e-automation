 ...
 */
public class PO35_SelectAllWithSearchFunctionality extends BasePageObject {

	private static final Logger LOGGER = LogManager.getLogger(PO35_SelectAllWithSearchFunctionality.class);

	public PO35_SelectAllWithSearchFunctionality() {
		super();
	}
	...
	private String getSearchSubstring(String screen) {
		if ("PM".equalsIgnoreCase(screen)) {
			return PO22_ValidateHCMSyncProfilesScreen_PM.jobProfileName.get();
		} else {
			return PO04_VerifyJobMappingPageComponents.jobnamesubstring.get();
		}
	}

	private String[] getSearchOptions(String screen) {
		if ("PM".equalsIgnoreCase(screen)) {
			return PO22_ValidateHCMSyncProfilesScreen_PM.SEARCH_PROFILE_NAME_OPTIONS;
		} else {
			return PO04_VerifyJobMappingPageComponents.SEARCH_SUBSTRING_OPTIONS;
		}
	}
