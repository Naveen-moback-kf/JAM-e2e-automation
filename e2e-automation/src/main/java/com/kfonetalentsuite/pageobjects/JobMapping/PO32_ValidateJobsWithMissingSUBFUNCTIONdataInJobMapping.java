...
public class PO32_ValidateJobsWithMissingSUBFUNCTIONdataInJobMapping extends BasePageObject {
	...
	public void find_job_in_jobs_missing_data_screen_where_subfunction_is_na() throws IOException {
		try {
			...
			PageObjectHelper.log(LOGGER, " REVERSE SCENARIO (Scenario 2): Finding job with missing Subfunction data...");
	...
	public void find_job_profile_in_job_mapping_page_where_subfunction_is_missing() throws IOException {
		try {
			...
			if (!matchFound) {
				...
				PageObjectHelper.log(LOGGER, " No profiles found with Function present but SubFunction missing");
