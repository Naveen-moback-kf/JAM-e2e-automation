...
public class PO32_ValidateJobsWithMissingSUBFUNCTIONdataInJobMapping extends DriverManager {
	 ...
	 * Used in Reverse flow: Job Mapping Missing Data screen find job with
	 * Subfunction=N/A - Runs INDEPENDENTLY of Forward scenario - If Forward
	 * scenario found a profile: Skip that profile to test a different one - If
	 * Forward scenario did NOT find a profile: Run normally and test any matching
	 * profile - If NO matching profile found in Reverse scenario: SKIP the Reverse
	 * scenario
	 */
	public void find_job_in_jobs_missing_data_screen_where_subfunction_is_na() throws IOException {
		try {
			...
			LOGGER.info(
					"REVERSE SCENARIO (Scenario 2): Running independently - will search for jobs with missing Subfunction");
			LOGGER.info("Looking for: Subfunction missing (ignoring Grade & Department data)");
			ExtentCucumberAdapter
					.addTestStepLog(" REVERSE SCENARIO (Scenario 2): Finding job with missing Subfunction data...");
			...
			if (forwardScenarioFoundProfile.get()) {
				LOGGER.info("Forward scenario found profile: " + forwardScenarioJobName.get() + " ("
						+ forwardScenarioJobCode.get() + ") - will skip it");
			} else {
			...
			if (preferredJobRow != null) {
				...
				if (cells.size() >= 4) {
					...
					ExtentCucumberAdapter.addTestStepLog(
							"   Function/Subfunction: " + functionSubfunction + " (Subfunction Missing)");
				...
				String skipMsg = "SKIPPING REVERSE SCENARIO: No jobs found with missing Subfunction in "
	...
	public void verify_job_profile_is_found_and_displayed_in_search_results() throws IOException {
		try {
			...
			LOGGER.info("Searching within search results for exact match: Job Name='" + targetJobName + "', Job Code='"
					+ targetJobCode + "'");
			...
			while (!jobFound && hasMoreRows) {
				...
				for (int i = totalRowsChecked; i < currentJobRows.size(); i++) {
					...
					try {
						...
						LOGGER.info("Row " + totalRowsChecked + ": '" + jobNameText + "' (" + jobCodeText + ")  Match: "
						...
						if (nameMatches && codeMatches) {
							...
							jobFound = true;
							LOGGER.info("Found EXACT matching job profile at row " + totalRowsChecked + ": "
									+ jobNameText + " (" + jobCodeText + ")");
							break;
						}
					} catch (Exception e) {
			...
			Assert.assertTrue(jobFound,
					"No exact matching job profile found in search results with Name='" + targetJobName + "' AND Code='"
			...
			ExtentCucumberAdapter
					.addTestStepLog("Successfully verified exact matching job profile is found in search results");
	 ...
	 */
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_subfunction_data()
			throws IOException {
		try {
			String fullJobName = jobDetailsFromMissingDataScreen.get("jobName");
	...
	/**
	 * Helper method to check if Subfunction is missing (Function has data but
	 * Subfunction is missing) Format: "Function | Subfunction" where: - "Value | -"
	 * = Function has data, Subfunction missing (SUBFUNCTION MISSING) - "Value |
	 * N/A" = Function has data, Subfunction missing (SUBFUNCTION MISSING) - "- | -"
	 * = Both empty (NOT SUBFUNCTION MISSING) - "Value | Value" = Both have data
	 * (NOT SUBFUNCTION MISSING)
	 */
	private boolean isSubfunctionMissing(String functionSubfunction, String context) {
		...
		if (functionParts.length >= 2) {
			...
			boolean subfunctionPartIsMissing = "-".equals(subfunctionPart) || "N/A".equalsIgnoreCase(subfunctionPart)
					|| subfunctionPart.isEmpty();
	...
	/**
	 * Sort Job Profiles by Department in Ascending order - FORWARD SCENARIO
	 * (Scenario 1): - Used in Forward flow to organize profiles
	 * (Function/Sub-function column doesn't support sorting) - Helps organize the
	 * profile list for more efficient searching Note: Function/Sub-function column
	 * doesn't have sorting capability, so we sort by Department instead
	 */
	public void sort_job_profiles_by_department_in_ascending_order() throws IOException {
		try {
			...
			ExtentCucumberAdapter
					.addTestStepLog("Sorting profiles by Department (ascending) to organize profile list...");
	...
	/**
	 * Find job profile with missing SUBFUNCTION in Job Mapping page - FORWARD
	 * SCENARIO (Scenario 1): - Used in Forward flow: Find job on Job Mapping page
	 * with missing Subfunction + info message - Searches Job Mapping screen for
	 * FIRST job with missing Subfunction and info message - Required conditions:
	 * Subfunction missing + Info message displayed (ignoring Grade/Department data)
	 */
	public void find_job_profile_in_job_mapping_page_where_subfunction_is_missing() throws IOException {
		try {
			...
			LOGGER.info(
					"========== FORWARD SCENARIO (Scenario 1): Searching for job with MISSING SUBFUNCTION ==========");
			LOGGER.info(
					"SEARCH CRITERIA: Function Missing = FALSE (Function must be present) AND SubFunction Missing = TRUE");
			LOGGER.info(
					"Looking for: info message + Function present + Subfunction missing (ignoring Grade & Department data)");
			ExtentCucumberAdapter.addTestStepLog(
					" FORWARD SCENARIO (Scenario 1): Finding FIRST job with Function present but SubFunction missing...");
			...
			while (scrollAttempt <= maxScrollAttempts && !matchFound) {
				...
				for (int i = lastCheckedIndex; i < infoMessageRows.size(); i++) {
					...
					try {
						...
						LOGGER.info("Profile " + profileNumber
								+ " - Function Missing = FALSE (present), SubFunction Missing = " + subfunctionIsMissing
								+ " (Required: TRUE)");
						...
						if (subfunctionIsMissing) {
							...
							ExtentCucumberAdapter.addTestStepLog("... Found suitable profile: " + jobName
			...
			if (!matchFound) {
				LOGGER.info(" No profiles with Function present but SubFunction missing found (checked "
						+ lastCheckedIndex + " profiles across " + scrollAttempt + " batches)");
				ExtentCucumberAdapter
						.addTestStepLog(" No profiles found with Function present but SubFunction missing");
