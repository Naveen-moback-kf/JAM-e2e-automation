...
public class PO12_ValidatePersistanceOfFilters extends BasePageObject {

	private static final Logger LOGGER = LogManager.getLogger(PO12_ValidatePersistanceOfFilters.class);
	...
	public void verify_applied_filters_persist_on_job_mapping_ui() {
		try {
			...
			String actualResultsCount = getElementText(SHOWING_RESULTS);
			waitForElement(CLEAR_FILTERS_BTN);

			if (actualResultsCount.equals(PO04_VerifyJobMappingPageComponents.intialResultsCount.get())) {
			...
			String actualCount = "";
			try {
				actualCount = driver.findElement(SHOWING_RESULTS).getText();
			} catch (Exception ex) {
	...
	public void user_is_in_job_mapping_page_with_grades_filters_applied() {
		...
		try {
			WebElement resultsElement = wait.until(ExpectedConditions.visibilityOfElementLocated(SHOWING_RESULTS));
	...
	public void user_is_in_job_mapping_page_with_mapping_status_filters_applied() {
		...
		try {
			String currentCount = getElementText(SHOWING_RESULTS);
			if (currentCount != null && !currentCount.isEmpty() && currentCount.contains("Showing")) {
				PO04_VerifyJobMappingPageComponents.initialFilteredResultsCount.set(currentCount);
			}
		} catch (Exception e) {
	...
	public void user_is_in_view_published_screen_with_grades_filters_applied() {
		...
		try {
			String currentCount = getElementText(SHOWING_RESULTS);
			if (currentCount != null && !currentCount.isEmpty() && currentCount.contains("Showing")) {
				if (PO04_VerifyJobMappingPageComponents.initialFilteredResultsCount.get() == null) {
					PO04_VerifyJobMappingPageComponents.initialFilteredResultsCount.set(currentCount);
				}
			}
		} catch (Exception e) {
