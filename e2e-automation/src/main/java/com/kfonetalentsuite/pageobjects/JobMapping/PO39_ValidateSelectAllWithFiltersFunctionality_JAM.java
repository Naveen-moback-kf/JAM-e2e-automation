...
public class PO39_ValidateSelectAllWithFiltersFunctionality_JAM {
	...
	public void apply_filter_and_verify_profiles_count_in_job_mapping_screen_for_feature39() {
		try {
			...
			try {
				...
				if (gradesOptions.isEmpty()) {
					...
					gradesOptions = driver.findElements(
							By.xpath("//div[@data-testid='dropdown-Grades']//..//label/input[@type='checkbox']"));
				}

				if (gradesOptions.isEmpty()) {
					LOGGER.warn("Strategy 2 failed, trying Strategy 3 for Grades options...");
					// Strategy 3: Broader search within Grades container
					gradesOptions = driver.findElements(By.xpath(
							"//div[@data-testid='dropdown-Grades']/following-sibling::div//input[@type='checkbox']"));
				}
				...
				if (gradesOptions.isEmpty()) {
					...
					gradesOptions = driver.findElements(By.xpath(
							"//div[@data-testid='dropdown-Grades']/parent::*/following-sibling::div//input[@type='checkbox']"));
				}
				...
				if (!gradesOptions.isEmpty()) {
					...
					try {
						...
						labelElement = firstCheckbox.findElement(
								By.xpath(".//following-sibling::label | .//parent::*/label | .//parent::label"));
						firstFilterValue.set(labelElement.getText().trim());
					} catch (Exception e) {
						// Fallback: Try to find any text within the checkbox container
						try {
							firstFilterValue.set(firstCheckbox.findElement(By.xpath(".//parent::*//div | .//parent::*"))
									.getText().trim());
						} catch (Exception ex) {
					...
					LOGGER.info("Found Grades option: " + firstFilterValue.get());
					...
					ExtentCucumberAdapter
							.addTestStepLog("Applied filter: " + firstFilterType.get() + " = " + firstFilterValue.get());
				} else {
					LOGGER.error(" No Grades options found after expanding dropdown - All 4 strategies failed");
					ScreenshotHandler.captureFailureScreenshot("no_grades_options_found",
				...
				try {
					...
					// DYNAMIC XPATH: Find all available Departments options without hardcoding
					// values
					// Strategy 1: Direct descendant approach
					var departmentsOptions = driver.findElements(
					...
					if (departmentsOptions.isEmpty()) {
						...
						// Strategy 2: Alternative approach with label elements
						departmentsOptions = driver.findElements(By.xpath(
								"//div[@data-testid='dropdown-Departments']//..//label/input[@type='checkbox']"));
					}
					...
					if (departmentsOptions.isEmpty()) {
						...
						departmentsOptions = driver.findElements(By.xpath(
								"//div[@data-testid='dropdown-Departments']/following-sibling::div//input[@type='checkbox']"));
					}
					...
					if (departmentsOptions.isEmpty()) {
						...
						// Strategy 4: Using parent container approach
						departmentsOptions = driver.findElements(By.xpath(
								"//div[@data-testid='dropdown-Departments']/parent::*/following-sibling::div//input[@type='checkbox']"));
					}
					...
					if (!departmentsOptions.isEmpty()) {
						...
						try {
							...
							// Fallback: Try to find any text within the checkbox container
							try {
								firstFilterValue.set(firstCheckbox
										.findElement(By.xpath(".//parent::*//div | .//parent::*")).getText().trim());
							} catch (Exception ex) {
								// Last resort: Get text from parent container
								firstFilterValue
						...
						LOGGER.info("Found Departments option: " + firstFilterValue.get());
						...
						LOGGER.error(
								" No Departments options found after expanding dropdown - All 4 strategies failed");
						ScreenshotHandler.captureFailureScreenshot("no_departments_options_found",
								new Exception("No options found"));
			...
			LOGGER.error(
					" Critical error applying filter - Method: apply_filter_and_verify_profiles_count_in_job_mapping_screen_for_feature39",
					e);
	...
	public void user_should_scroll_down_to_view_last_filtered_result_in_job_mapping_screen_for_feature39() {
		try {
			...
			if (expectedTotal > 0) {
				...
				LOGGER.info(
						"Calculated maxScrollAttempts: " + maxScrollAttempts + " (for " + expectedTotal + " profiles)");
			} else {
				LOGGER.warn(
						" Using default maxScrollAttempts: " + maxScrollAttempts + " (expected total not available)");
			}
			...
			while (scrollAttempts < maxScrollAttempts && !allResultsLoaded) {
				...
				var allRows = driver.findElements(By.xpath(
						"//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input"));
				...
				LOGGER.debug("Scroll attempt " + scrollAttempts + ": " + previousCount + " → " + currentCount
						+ " profiles (+" + newlyLoadedProfiles + " new)");

				// If we know the expected total, stop as soon as we reach it
				if (expectedTotal > 0 && currentCount >= expectedTotal) {
					allResultsLoaded = true;
					LOGGER.info("Loaded " + currentCount + " filtered results after " + scrollAttempts
							+ " scrolls (expected " + expectedTotal + ")");
					break;
				}
			...
			LOGGER.error(
					" Issue scrolling to view filtered results - Method: user_should_scroll_down_to_view_last_filtered_result_in_job_mapping_screen_for_feature39",
					e);
	...
	public void user_should_validate_all_filtered_results_match_the_applied_filter_in_job_mapping_screen_for_feature39() {
		try {
			...
			// IMPORTANT: Use org-job-container to get ONLY Organization Jobs table rows
			var profileNameElements = driver.findElements(
			...
			// 1. Identify the column index for the filter type (Grades = column X,
			// Departments = column Y)
			...
			for (WebElement element : profileNameElements) {
				try {
					...
					// Basic validation: ensure profile name is not empty and contains job code
					// format
			...
			if (totalResults == 0) {
				LOGGER.warn(" No profile elements found - Filter may have returned 0 results");
				ExtentCucumberAdapter.addTestStepLog(" No profile elements found - Filter may have returned 0 results");
			} else if (nonMatchingResults == 0) {
				LOGGER.info(" All " + totalResults + " results validated for filter: " + firstFilterType.get() + " = "
						+ firstFilterValue.get());
				ExtentCucumberAdapter.addTestStepLog(" All " + totalResults + " results validated for filter: "
						+ firstFilterType.get() + " = " + firstFilterValue.get());
			} else {
				LOGGER.warn(" " + nonMatchingResults + " of " + totalResults
						+ " results could NOT be validated for filter: " + firstFilterType.get() + " = " + firstFilterValue.get());
				ExtentCucumberAdapter.addTestStepLog(" " + nonMatchingResults + " results could not be validated");
			}

		} catch (Exception e) {
			LOGGER.error(
					" Issue validating filtered results - Method: user_should_validate_all_filtered_results_match_the_applied_filter_in_job_mapping_screen_for_feature39",
					e);
	...
	public void user_is_in_job_mapping_page_with_selected_filter_results_for_feature39() {
		try {
			...
			// Count ACTUAL selected profiles by counting CHECKED checkboxes (more reliable
			// than bg-blue-50 class)
			// IMPORTANT: Use org-job-container to get ONLY Organization Jobs table rows
			var allCheckboxes = driver.findElements(By.xpath(
					"//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input[@type='checkbox']"));
			...
			filterResultsCount.set(checkedCount); // This is the actual number of selected profiles
			int totalVisibleProfiles = allCheckboxes.size();
			int unselectedProfiles = totalVisibleProfiles - filterResultsCount.get(); // Profiles that were not selected
			...
			if (filterResultsCount.get() == 0) {
				LOGGER.error(
						" CRITICAL: filterResultsCount is 0 after 'Select All' - this indicates a timing or detection issue!");
				LOGGER.error("   Total checkboxes found: " + totalVisibleProfiles);
				LOGGER.error("   Checked checkboxes: " + checkedCount);
				ScreenshotHandler.captureFailureScreenshot("filterResultsCount_is_zero",
						new Exception("filterResultsCount is 0"));
			}
	 ...
	 * VERIFICATION LOGIC:
	 * 
	 * WHAT filterResultsCount STORES (set in previous step): - Number of profiles
	 * selected AFTER applying first filter + "Select All" - Example: Filter "Grades
	 * = 1" 5 profiles match "Select All" filterResultsCount = 5
	 * 
	 * WHAT THIS METHOD DOES: 1. Filters have been cleared ALL profiles are now
	 * visible (not just the 5) 2. Scroll through ALL profiles to load them (lazy
	 * loading requires scrolling) 3. Count how many profiles are STILL selected 4.
	 * VERIFY: Selected count should equal filterResultsCount (only the original 5
	 * should remain selected)
	 * 
	 * WHY WE SCROLL: - Job Mapping uses lazy loading (profiles load as you scroll)
	 * - We must scroll through all profiles to verify none of the newly visible
	 * profiles got selected - If any profile beyond the original 5 is selected, the
	 * test FAILS (fail-fast mechanism)
	 */
	public void verify_only_filtered_profiles_are_selected_after_clearing_all_filters_in_job_mapping_screen() {
		...
		try {
			...
			while (scrollAttempts < maxScrollAttempts && !allProfilesLoaded) {
				...
				// IMPORTANT: Use org-job-container to get ONLY Organization Jobs table rows
				var allProfileRows = driver.findElements(By.xpath(
						"//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input"));
				...
				var currentCheckboxes = driver.findElements(By.xpath(
						"//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input[@type='checkbox']"));
				...
				if (currentSelectedCount > filterResultsCount.get()) {
					...
					LOGGER.warn(" FAIL-FAST at scroll " + scrollAttempts + ": Found " + currentSelectedCount
							+ " selected profiles (expected " + filterResultsCount.get() + "), " + extra
			...
			if (actualSelectedCount == 0) {
				// Count checked checkboxes (more reliable than bg-blue-50 class)
				var finalCheckboxes = driver.findElements(By.xpath(
						"//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input[@type='checkbox']"));
			...
			if (firstFilterType.equals("Grades")) {
				...
				try {
					...
					while (gradesOptions.isEmpty() && retryCount < maxRetries) {
						...
						if (gradesOptions.isEmpty()) {
							...
							// Strategy 2: Alternative approach with label elements
							gradesOptions = driver.findElements(By
									.xpath("//div[@data-testid='dropdown-Grades']//..//label/input[@type='checkbox']"));
						}
						...
						if (gradesOptions.isEmpty() && retryCount < maxRetries) {
							LOGGER.warn("No Grades options found on attempt " + retryCount
					...
					if (!gradesOptions.isEmpty()) {
						...
						LOGGER.info("Selecting Grades option at index " + optionIndex + " (total options: "
								+ gradesOptions.size() + ")");
						...
						try {
							...
							if (!secondFilterValue.get().isEmpty()) {
								LOGGER.info(" Extracted label using Strategy 1 (label element): " + secondFilterValue.get());
							}
						} catch (Exception e) {
						...
						if (secondFilterValue.get().isEmpty()) {
							try {
								...
								if (!secondFilterValue.get().isEmpty()) {
									LOGGER.info(" Extracted label using Strategy 2 (parent container): "
											+ secondFilterValue);
								}
							} catch (Exception e) {
						...
						if (secondFilterValue.get().isEmpty()) {
							try {
								WebElement textDiv = selectedOption
										.findElement(By.xpath("./parent::*//div | ./parent::*"));
								secondFilterValue.set(textDiv.getText().trim());
								if (!secondFilterValue.get().isEmpty()) {
									LOGGER.info(" Extracted label using Strategy 3 (text div): " + secondFilterValue.get());
								}
							} catch (Exception e) {
						...
						if (secondFilterValue.get().isEmpty()) {
							secondFilterValue.set("Grades_Option_" + (optionIndex + 1));
							LOGGER.warn(
									" All label extraction strategies failed, using fallback: " + secondFilterValue.get());
						}

						LOGGER.info("Found Grades option: '" + secondFilterValue.get() + "' (different from first: '"
								+ firstFilterValue.get() + "')");

						// Re-find the element to avoid stale reference
						// Build XPath to find the specific option by index
						String optionXPath = "(//div[@data-testid='dropdown-Grades']//..//input[@type='checkbox'])["
								+ (optionIndex + 1) + "]";
						...
						if (!clickSucceeded) {
							ScreenshotHandler.captureFailureScreenshot("failed_to_click_grades_alternative_filter",
						...
						try {
							...
							if (resultsAfterFilter.contains("0 of")) {
								LOGGER.warn(
										" Second filter returned 0 results - this is expected if filters are mutually exclusive");
							}
						} catch (Exception e) {
							LOGGER.debug("Could not verify results count after filter: " + e.getMessage());
						}

						LOGGER.info(" Applied same filter type with different value: " + secondFilterType.get() + " = "
								+ secondFilterValue.get() + " (first was: " + firstFilterValue.get() + ")");
						ExtentCucumberAdapter.addTestStepLog(" Using: " + secondFilterType.get() + " = " + secondFilterValue.get()
								+ " (ensures unique results)");
					} else {
						LOGGER.error(" No Grades options found after expanding dropdown - All 4 strategies failed");
						ScreenshotHandler.captureFailureScreenshot("no_grades_options_alternative",
								new Exception("No options found"));
					...
					Assert.fail("Failed to apply Grades filter for alternative validation: "
							+ gradesException.getMessage());
				}
			} else {
				// If first was Departments, apply Departments again with different value
				secondFilterType.set("Departments");
				...
				try {
					...
					while (departmentsOptions.isEmpty() && retryCount < maxRetries) {
						retryCount++;
						LOGGER.info(
								"Attempt " + retryCount + "/" + maxRetries + ": Looking for Departments options...");

						// DYNAMIC XPATH: Find all available Departments options without hardcoding
						// values
						// Strategy 1: Direct descendant approach
						departmentsOptions = driver.findElements(
								By.xpath("//div[@data-testid='dropdown-Departments']//..//input[@type='checkbox']"));
						...
						if (departmentsOptions.isEmpty() && retryCount < maxRetries) {
							LOGGER.warn("No Departments options found on attempt " + retryCount
									+ ", waiting 1 second before retry...");
							Thread.sleep(1000);
						}
					}
					...
					if (!departmentsOptions.isEmpty()) {
						...
						LOGGER.info("Selecting Departments option at index " + optionIndex + " (total options: "
								+ departmentsOptions.size() + ")");
						...
						// Fallback: Use descriptive placeholder
						if (secondFilterValue.get().isEmpty()) {
							secondFilterValue.set("Departments_Option_" + (optionIndex + 1));
							LOGGER.warn(
									" All label extraction strategies failed, using fallback: " + secondFilterValue.get());
						}

						LOGGER.info("Found Departments option: '" + secondFilterValue.get() + "' (different from first: '"
								+ firstFilterValue.get() + "')");

						// Re-find the element to avoid stale reference
						// Build XPath to find the specific option by index
						String optionXPath = "(//div[@data-testid='dropdown-Departments']//..//input[@type='checkbox'])["
								+ (optionIndex + 1) + "]";
						...
						if (!clickSucceeded) {
							ScreenshotHandler.captureFailureScreenshot("failed_to_click_departments_alternative_filter",
									new Exception("Click failed"));
						...
						LOGGER.error(
								" No Departments options found after expanding dropdown - All 4 strategies failed");
						ScreenshotHandler.captureFailureScreenshot("no_departments_options_alternative",
								new Exception("No options found"));
						throw new Exception("No Departments filter options found for alternative validation");
					}
				} catch (Exception departmentsException) {
					LOGGER.error(" Failed to apply Departments filter for alternative validation",
							departmentsException);
					ScreenshotHandler.captureFailureScreenshot("departments_filter_failed_alternative",
							departmentsException);
					Assert.fail("Failed to apply Departments filter for alternative validation: "
							+ departmentsException.getMessage());
				}
			}

		} catch (Exception e) {
			LOGGER.error(
					" Critical error applying different filter - Method: apply_different_filter_for_alternative_validation_in_job_mapping_screen_for_feature39",
					e);
	...
	private int checkNewProfilesForInvalidSelectionsForFilter(int startIndex, int endIndex, String firstFilterType,
		...
		try {
			...
			// IMPORTANT: Use org-job-container to get ONLY Organization Jobs table rows
			// (not Matched Success Profiles table)
			var allCheckboxes = driver.findElements(By.xpath(
					"//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input[@type='checkbox']"));
			...
			for (int checkboxIndex = 0; checkboxIndex < allCheckboxes.size(); checkboxIndex++) {
				try {
					...
					if (rowIndex < allRows.size()) {
						...
						LOGGER.warn(" Skipped checkbox at index " + checkboxIndex + " (row index " + rowIndex
								+ " out of bounds)");
						continue;
					}
					...
					try {
						...
						LOGGER.warn(" Skipped profile at checkbox index " + checkboxIndex + ", row " + (rowIndex + 1)
					...
					invalidCount++;
					LOGGER.warn("- INVALID selection at profile #" + (checkboxIndex + 1) + ": '" + jobName
							+ "' (should NOT be selected in " + secondFilterType + " = " + secondFilterValue + ")");
	 ...
	 */
	public void scroll_down_to_load_all_second_filter_results_in_job_mapping_screen_for_feature39()
			throws InterruptedException {
		String secondFilterType = PO39_ValidateSelectAllWithFiltersFunctionality_JAM.secondFilterType.get();
		...
		try {
			...
			ExtentCucumberAdapter.addTestStepLog("Second filter: " + expectedTotal + " profiles");

			// Get initial profile count using checkbox XPath
			String checkboxXpath = "//div[@id='org-job-container']//tbody//tr//td[1][contains(@class,'whitespace')]//input[@type='checkbox']";
			int previousProfileCount = driver.findElements(By.xpath(checkboxXpath)).size();
			int currentProfileCount = previousProfileCount;
			int scrollAttempts = 0;
			int maxScrollAttempts = 50; // Safety limit
			int noChangeCount = 0;

			LOGGER.info("Starting scroll to load all second filter results. Initial loaded: " + previousProfileCount);
			...
			ExtentCucumberAdapter.addTestStepLog("Loaded " + currentProfileCount + " of " + expectedTotal + " second filter profiles");
			...
			ExtentCucumberAdapter.addTestStepLog(" Error loading second filter results");
			Assert.fail("Error loading second filter results: " + e.getMessage());
		}
	}

	/**
	 * Verifies that all loaded profiles from the second filter are NOT selected.
	 * These profiles should NOT be selected because they don't match the first filter.
	 */
	public void verify_all_loaded_profiles_in_second_filter_are_not_selected_in_job_mapping_screen_for_feature39() {
		String firstFilterType = PO39_ValidateSelectAllWithFiltersFunctionality_JAM.firstFilterType.get();
		String firstFilterValue = PO39_ValidateSelectAllWithFiltersFunctionality_JAM.firstFilterValue.get();
		String secondFilterType = PO39_ValidateSelectAllWithFiltersFunctionality_JAM.secondFilterType.get();
		String secondFilterValue = PO39_ValidateSelectAllWithFiltersFunctionality_JAM.secondFilterValue.get();
		...
		try {
			...
			LOGGER.info("Second filter validation: {} total loaded, {} selected", totalLoaded, selectedCount);
			
			// If any profiles are selected, check if they belong to first filter (valid) or second filter (invalid)
			if (selectedCount > 0) {
				...
				int invalidCount = checkNewProfilesForInvalidSelectionsForFilter(0, totalRowCount, firstFilterType,
			...
			ExtentCucumberAdapter.addTestStepLog(" Error verifying second filter profiles");
