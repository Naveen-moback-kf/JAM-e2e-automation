...
public class PO39_ValidateSelectAllWithFiltersFunctionality_JAM {
	...
	public static ThreadLocal<String> firstFilterType = ThreadLocal.withInitial(() -> ""); // e.g., "Grades", "Departments", "Functions"
	public static ThreadLocal<String> firstFilterValue = ThreadLocal.withInitial(() -> ""); // e.g., "M3", "HR", "Finance"
			...
			try {
				...
				if (!gradesOptions.isEmpty()) {
					...
					try {
						...
						try {
							firstFilterValue.set(firstCheckbox.findElement(By.xpath(".//parent::*//div | .//parent::*")).getText().trim());
						} catch (Exception ex) {
	...
	public void user_should_scroll_down_to_view_last_filtered_result_in_job_mapping_screen_for_feature39() {
		try {
			...
			if (expectedTotal > 0) {
				...
				LOGGER.warn(" Using default maxScrollAttempts: " + maxScrollAttempts + " (expected total not available)");
			}
				...
				try {
					...
					try {
						...
						if (resultsAfterFilter.contains("0 of")) {
							LOGGER.warn(" Second filter returned 0 results - this is expected if filters are mutually exclusive");
						}
					} catch (Exception e) {
			...
			for (int checkboxIndex = 0; checkboxIndex < allCheckboxes.size(); checkboxIndex++) {
				try {
					...
					if (rowIndex < allRows.size()) {
						...
						LOGGER.warn(" Skipped checkbox at index " + checkboxIndex + " (row index " + rowIndex + " out of bounds)");
						continue;
					}
					...
					try {
						...
						LOGGER.warn(" Skipped profile at checkbox index " + checkboxIndex + ", row " + (rowIndex + 1) + " (could not extract title)");
						continue;
					}
