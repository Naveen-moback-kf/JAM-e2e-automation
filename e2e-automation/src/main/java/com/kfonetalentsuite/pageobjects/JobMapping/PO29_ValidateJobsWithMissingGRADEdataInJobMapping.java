...
public class PO29_ValidateJobsWithMissingGRADEdataInJobMapping extends DriverManager {
	 ...
	 */
	public void find_job_in_jobs_missing_data_screen_where_grade_is_na_but_department_or_function_subfunction_is_not_na()
			throws IOException {
		try {
			...
			LOGGER.info(
					"REVERSE SCENARIO (Scenario 2): Forward scenario found profile - will search for jobs with missing Grade");
			LOGGER.info("Looking for: Grade missing (ignoring Department & Function data)");
			ExtentCucumberAdapter
					.addTestStepLog(" REVERSE SCENARIO (Scenario 2): Finding job with missing Grade data...");
			...
			if (preferredJobRow != null) {
				...
				ExtentCucumberAdapter
						.addTestStepLog("... Found job profile where Grade is missing - proceeding with validation");
	...
	/**
	 * Check if Missing Data screen is currently open FIXED: Using specific XPaths
	 * to avoid false positives on Job Mapping page
	 */
	private boolean isMissingDataScreenOpen() {
		try {
			...
			try {
				WebElement missingDataTitle = driver.findElement(By.xpath(
			...
			// Additional specific check: Look for the "Reupload Jobs" page close button
			// (specific to Missing Data screen)
			try {
				if (closeReuploadJobsPageButton != null && closeReuploadJobsPageButton.isDisplayed()) {
					...
					if (currentUrl != null
							&& (currentUrl.contains("missing-data") || currentUrl.contains("reupload"))) {
			...
			// If we can find Job Mapping page elements, we're definitely NOT on Missing
			// Data screen
			try {
				...
				// Job Mapping indicator not found - but this doesn't mean we're on Missing Data
				// screen
			}

			// Default: if we can't confirm Missing Data screen with specific indicators,
			// assume we're NOT on it
			return false;
	...
	/**
	 * Verify Info Message is displayed on searched profile Optimized using
	 * JavaScript for instant checking
	 */
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_grade_data()
			throws IOException {
		try {
			String fullJobName = jobDetailsFromMissingDataScreen.get("jobName");
			...
			Assert.assertTrue(infoMessageFound,
					"Info message not found on searched profile indicating missing Grade data");
	...
	/**
	 * Sort Job Profiles by Organization Grade in Ascending order - REVERSE SCENARIO
	 * (Scenario 2): - Used in Reverse flow to get profiles with missing Grade first
	 * (Grade="-" or empty) - Helps locate jobs with info messages about missing
	 * Grade data more efficiently
	 */
	public void sort_job_profiles_by_organization_grade_in_ascending_order() throws IOException {
		try {
			...
			ExtentCucumberAdapter
					.addTestStepLog("Sorting profiles by Grade (ascending) to get missing Grade profiles first...");

			// Look for Grade column header to sort
			List<WebElement> gradeHeaders = driver
					.findElements(By.xpath("//th[contains(text(), 'Grade') or contains(text(), 'grade')]"));

			if (gradeHeaders.isEmpty()) {
				// Try alternative locators for grade header
				gradeHeaders = driver
						.findElements(By.xpath("//th[contains(@class, 'grade') or contains(@data-field, 'grade')]"));
			}
			...
			if (!gradeHeaders.isEmpty()) {
				...
				ExtentCucumberAdapter
						.addTestStepLog("... Sorted profiles by Grade - missing Grade profiles should appear first");

			} else {
				LOGGER.warn("Grade column header not found, proceeding without sorting");
				ExtentCucumberAdapter
						.addTestStepLog(" Grade column not found for sorting, proceeding with current order");
			}
	...
	/**
	 * Find job profile with missing GRADE in Job Mapping page - FORWARD SCENARIO
	 * (Scenario 1): - Used in Forward flow: Find job on Job Mapping page with
	 * missing Grade + info message - Searches Job Mapping screen for FIRST job with
	 * missing Grade and info message - Required conditions: Grade missing + Info
	 * message displayed (ignoring Department/Function data)
	 */
	public void find_job_profile_in_job_mapping_page_where_grade_is_missing() throws IOException {
		try {
			...
			LOGGER.info(
					"FORWARD SCENARIO (Scenario 1): Searching for first job with info message where Grade is missing");
			LOGGER.info("Looking for: info message + Grade missing (ignoring Department & Function data)");
			ExtentCucumberAdapter.addTestStepLog(
					" FORWARD SCENARIO (Scenario 1): Finding FIRST job with info message and missing Grade...");
			...
			while (scrollAttempt <= maxScrollAttempts && !matchFound) {
				...
				for (int i = lastCheckedIndex; i < infoMessageRows.size(); i++) {
					...
					try {
						...
						LOGGER.info("Profile " + profileNumber + " - Grade Missing: " + gradeIsMissing
								+ " (Required: TRUE)");
						...
						if (gradeIsMissing) {
							...
							ExtentCucumberAdapter
									.addTestStepLog("... Found suitable profile: " + jobName + " - Grade missing");
			...
			if (!matchFound) {
				LOGGER.info(" No profiles with missing Grade found (checked " + lastCheckedIndex + " profiles across "
						+ scrollAttempt + " batches)");
				ExtentCucumberAdapter.addTestStepLog(" No profiles found with missing Grade");

				String skipMsg = "SKIPPING SCENARIO: No profiles found with missing Grade (checked " + lastCheckedIndex
						+ " profiles)";
	...
	public void extract_job_details_from_found_profile_in_job_mapping_page() throws IOException {
		try {
			...
			try {
				List<WebElement> jobNameElements = foundJobRow
						.findElements(By.xpath(".//td[2]//div | .//td[position()=2]//div"));
				if (!jobNameElements.isEmpty()) {
					String jobNameCodeText = jobNameElements.get(0).getAttribute("textContent").trim();
			...
			try {
				List<WebElement> gradeElements = foundJobRow
						.findElements(By.xpath(".//td[3]//div | .//td[position()=3]//div"));
				if (!gradeElements.isEmpty()) {
			...
			try {
				List<WebElement> departmentElements = foundJobRow
						.findElements(By.xpath(".//td[4]//div | .//td[position()=4]//div"));
				if (!departmentElements.isEmpty()) {
					department = normalizeFieldValue(departmentElements.get(0).getAttribute("textContent").trim(),
							"Department");
				}
			} catch (Exception e) {
			...
			if (foundJobRow == null) {
				...
				if (expectedJobCode != null && !expectedJobCode.isEmpty()) {
					...
					LOGGER.error("Job Mapping: Found job '" + searchTerm + "' with code '" + expectedJobCode
							+ "' and missing Grade + info message");
	 ...
	 * results Enhanced to validate both job name and job code match
	 */
			...
			if (foundJobRow.isDisplayed()) {
				...
				if (cells.size() >= 1) {
					...
					if (expectedJobCode != null && !expectedJobCode.isEmpty() && !jobCodeInRow.isEmpty()) {
						if (expectedJobCode.equals(jobCodeInRow)) {
							LOGGER.info(
									"... Job profile is displayed in Missing Data screen search results --- Automation Found "
											+ cleanJobNameInRow + " (" + jobCodeInRow
											+ ") as correct match with matching job code");
							ExtentCucumberAdapter
									.addTestStepLog("... Job profile verified in Missing Data screen - Name: "
											+ cleanJobNameInRow + ", Code: " + jobCodeInRow + " (EXACT MATCH)");
						} else {
							String errorMsg = " BUG: Job found but job code mismatch - Expected: " + expectedJobCode
									+ ", Found: " + jobCodeInRow + " for job: " + cleanJobNameInRow;
							LOGGER.error(errorMsg);
							LOGGER.error(
									"This indicates multiple jobs with same name but different codes exist in the system");
							ExtentCucumberAdapter.addTestStepLog(" " + errorMsg);
							throw new IOException(errorMsg);
						}
					} else {
						LOGGER.info(
								"... Job profile is displayed in Missing Data screen search results --- but in Missing data screen Automation Found "
										+ cleanJobNameInRow + " (" + jobCodeInRow
										+ ") as correct match but here Job Code is different...Could you please fix this issue?");
						ExtentCucumberAdapter.addTestStepLog("... Job profile verified in Missing Data screen - Name: "
								+ cleanJobNameInRow + (jobCodeInRow.isEmpty() ? "" : ", Code: " + jobCodeInRow)
								+ " (Code validation not possible)");
					}
				}
			} else {
	...
	public void extract_job_details_from_found_profile_in_jobs_missing_data_screen() throws IOException {
		try {
			...
			if (cells.size() >= 4) {
				...
				LOGGER.info("Successfully extracted job details from Missing Data screen: " + jobName
						+ (jobCode.isEmpty() ? "" : " (Code: " + jobCode + ")"));
	 ...
	 * screen Used by both Forward and Reverse scenarios for cross-validation
	 */
		...
		try {
			...
			if (!codeFromJobMapping.isEmpty() && !codeFromMissingData.isEmpty()) {
				if (codeFromJobMapping.equals(codeFromMissingData)) {
					...
					String errorMsg = " BUG: Job Code mismatch - JobMapping='" + codeFromJobMapping
							+ "' vs MissingData='" + codeFromMissingData
							+ "' - This indicates multiple jobs with same name but different codes";
					LOGGER.error(errorMsg);
					ExtentCucumberAdapter.addTestStepLog(" " + errorMsg);
					Assert.fail(errorMsg);
				}
			} else {
				LOGGER.warn("Job Code comparison skipped - one or both values are empty (JobMapping: '"
						+ codeFromJobMapping + "', MissingData: '" + codeFromMissingData + "')");
				ExtentCucumberAdapter
						.addTestStepLog(" Job Code validation not possible - codes not available in both screens");
			}
