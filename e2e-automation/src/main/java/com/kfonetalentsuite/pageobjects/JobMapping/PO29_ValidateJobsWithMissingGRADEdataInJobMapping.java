...
public class PO29_ValidateJobsWithMissingGRADEdataInJobMapping extends BasePageObject {

	private static final Logger LOGGER = LogManager.getLogger(PO29_ValidateJobsWithMissingGRADEdataInJobMapping.class);
	...
	public void verify_user_is_navigated_to_jobs_with_missing_data_screen() throws IOException {
		try {
			...
			PageObjectHelper.log(LOGGER, "- Failed to verify Jobs with Missing Data screen: " + e.getMessage());
	...
	public void find_job_in_jobs_missing_data_screen_where_grade_is_na_but_department_or_function_subfunction_is_not_na()
		...
		try {
			...
			PageObjectHelper.log(LOGGER, " REVERSE SCENARIO (Scenario 2): Finding job with missing Grade data...");
			...
			if (preferredJobRow != null) {
				...
				PageObjectHelper.log(LOGGER, "Found job profile where Grade is missing - proceeding with validation");
				...
				if (cells.size() >= 4) {
					...
					PageObjectHelper.log(LOGGER, "   Grade: " + grade + " (Missing)");
					PageObjectHelper.log(LOGGER, "   Department: " + department);
	...
	public void verify_user_is_back_on_job_mapping_page() throws IOException {
		try {
			...
			Assert.assertTrue(findElement(SEARCH_BOX).isDisplayed(), "Job search input not visible - not on Job Mapping page");
	...
	public void search_for_the_extracted_job_profile_by_name_in_job_mapping_page() throws IOException {
		try {
			wait.until(ExpectedConditions.elementToBeClickable(findElement(SEARCH_BOX)));
			...
			findElement(SEARCH_BOX).sendKeys(searchTerm);
	...
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_grade_data()
		...
		try {
			...
			PageObjectHelper.log(LOGGER, "Info Message verified for profile with missing Grade data");
	...
	public void sort_job_profiles_by_organization_grade_in_ascending_order() throws IOException {
		try {
			PageObjectHelper.log(LOGGER, "Sorting profiles by Grade (ascending) to get missing Grade profiles first...");
			...
			if (!gradeHeaders.isEmpty()) {
				...
				PageObjectHelper.log(LOGGER, "Sorted profiles by Grade - missing Grade profiles should appear first");

			} else {
				PageObjectHelper.log(LOGGER, " Grade column not found for sorting, proceeding with current order");
			}
	...
	public void find_job_profile_in_job_mapping_page_where_grade_is_missing() throws IOException {
		try {
			...
			while (scrollAttempt <= maxScrollAttempts && !matchFound) {
				...
				for (int i = lastCheckedIndex; i < infoMessageRows.size(); i++) {
					...
					try {
						...
						if (gradeIsMissing) {
							PageObjectHelper.log(LOGGER, "Found suitable profile: " + jobName + " - Grade missing");
			...
			if (!matchFound) {
				...
				PageObjectHelper.log(LOGGER, " No profiles found with missing Grade");
		...
		try {
			...
			if (foundJobRow.isDisplayed()) {
				...
				if (cells.size() >= 1) {
					...
					if (expectedJobCode != null && !expectedJobCode.isEmpty() && !jobCodeInRow.isEmpty()) {
						if (expectedJobCode.equals(jobCodeInRow)) {
							PageObjectHelper.log(LOGGER, "Job profile verified in Missing Data screen - Name: "
		...
		try {
			...
			if (!codeFromJobMapping.isEmpty() && !codeFromMissingData.isEmpty()) {
				if (codeFromJobMapping.equals(codeFromMissingData)) {
					PageObjectHelper.log(LOGGER, "... Job Code verification passed: " + codeFromJobMapping);
				} else {
				...
				PageObjectHelper.log(LOGGER, " Job Code validation not possible - codes not available in both screens");
			}
			...
			PageObjectHelper.log(LOGGER, "Successfully verified job details match between screens");
