...
public class PO29_ValidateJobsWithMissingGRADEdataInJobMapping extends DriverManager {
				...
				try {
					...
					for (WebElement closeButton : closeButtons) {
						if (closeButton.isDisplayed() && closeButton.isEnabled()) {
							...
							LOGGER.info("Force closed Missing Data screen using fallback method");
							PerformanceUtils.safeSleep(driver, 2000);
							return;
						}
					}
	...
	/**
	 * Verify Info Message is displayed on searched profile
	 * Optimized using JavaScript for instant checking
	 */
	public void verify_info_message_is_displayed_on_searched_profile_indicating_missing_grade_data() throws IOException {
		try {
			...
			ExtentCucumberAdapter.addTestStepLog("Info Message verified for profile with missing Grade data");
	...
	public void sort_job_profiles_by_organization_grade_in_ascending_order() throws IOException {
		try {
			...
			if (!gradeHeaders.isEmpty()) {
				...
				ExtentCucumberAdapter.addTestStepLog(" Grade column not found for sorting, proceeding with current order");
			}
...
/**
	...
	public void extract_job_details_from_found_profile_in_job_mapping_page() throws IOException {
		try {
			...
			// Extract Job Name and Code from column 2 (Feature 28 approach) - OPTIMIZED
			try {
				List<WebElement> jobNameElements = foundJobRow.findElements(
					By.xpath(".//td[2]//div | .//td[position()=2]//div")
				);
				if (!jobNameElements.isEmpty()) {
					String jobNameCodeText = jobNameElements.get(0).getAttribute("textContent").trim();
			...
			// Extract Grade from column 3 (Feature 28 approach) - OPTIMIZED
			try {
				List<WebElement> gradeElements = foundJobRow.findElements(
					By.xpath(".//td[3]//div | .//td[position()=3]//div")
				);
				if (!gradeElements.isEmpty()) {
					grade = normalizeFieldValue(gradeElements.get(0).getAttribute("textContent").trim(), "Grade");
				}
			} catch (Exception e) {
				LOGGER.warn("Could not extract grade using Feature 28 logic: " + e.getMessage());
			}
			
			// Extract Department from column 4 (Feature 28 approach) - OPTIMIZED
			try {
				List<WebElement> departmentElements = foundJobRow.findElements(
					By.xpath(".//td[4]//div | .//td[position()=4]//div")
				);
				if (!departmentElements.isEmpty()) {
					department = normalizeFieldValue(departmentElements.get(0).getAttribute("textContent").trim(), "Department");
				}
			} catch (Exception e) {
	...
	public void verify_all_job_details_match_between_job_mapping_page_and_jobs_missing_data_screen() throws IOException {
		try {
			...
			if (!codeFromJobMapping.isEmpty() && !codeFromMissingData.isEmpty()) {
				...
				ExtentCucumberAdapter.addTestStepLog(" Job Code validation not possible - codes not available in both screens");
			}
