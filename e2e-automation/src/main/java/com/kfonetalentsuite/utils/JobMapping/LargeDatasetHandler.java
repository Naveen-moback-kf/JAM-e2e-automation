...
/**
 * Utility class for handling large datasets in web automation Prevents browser
 * unresponsiveness during extensive scrolling operations
 * 
...
public class LargeDatasetHandler {
	...
	/**
	 * Performs throttled scrolling with browser stabilization to prevent
	 * unresponsiveness
	 * 
	 ...
	 */
	public static int throttledScrollToLoadData(WebDriver driver, WebDriverWait wait, int maxScrolls, int scrollDelay,
			By spinnerLocator) {
	...
	/**
	 * Extracts data directly via JavaScript, bypassing UI scrolling Use this for
	 * maximum performance when you don't need to interact with UI elements
	 * 
	 * @param driver          WebDriver instance
	...
	public static List<Map<String, String>> extractDataViaJavaScript(WebDriver driver, String rowSelector,
		...
		try {
			List<Map<String, String>> extractedData = (List<Map<String, String>>) js.executeScript(script.toString());
	 ...
	 */
	public static int scrollToLoadRecordCount(WebDriver driver, WebDriverWait wait, int targetRecordCount,
			String recordSelector, int maxScrollAttempts) {
		...
		try {
			while (scrollAttempts < maxScrollAttempts) {
				...
				if (scrollAttempts % 10 == 0) {
					LOGGER.info(" Progress: {} records loaded, {} scrolls performed", loadedRecords, scrollAttempts);
				}
			}

			LOGGER.info("... Scroll loading completed. Records loaded: {}, Scrolls: {}", loadedRecords, scrollAttempts);
