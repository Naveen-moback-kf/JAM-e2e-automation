...
public class Utilities {
	...
	public List<String> randomSelection(List<WebElement> AllCheckboxes, int count, By path, WebDriver driver) {
		...
		try {
			...
			WebElement ignored : AllCheckboxes) {
				while (i <= count) {
	...
	public List<String> randomFilterSelection(List<WebElement> AllCheckboxes, int count, By path, WebDriver driver,
		...
		try {
			...
			WebElement ignored : AllCheckboxes) {
				while (i <= count) {
					...
					if (!options.isEmpty()) {
						int list = rand.nextInt(options.size());
						String checkbox = options.get(list).getAttribute("class");
