 ...
 * - Always logs to Log4j (for file-based logs) - Conditionally logs to Extent
 * Reports (based on configuration flag) - Provides consistent logging patterns
 * across the framework
 * 
 * Configuration: - Set extent.reporting.enabled=true in config.properties to
 * enable Extent logging - Set extent.reporting.enabled=false to disable Extent
 * logging (Log4j still works)
 * 
 * Benefits: - Single point of control for Extent reporting - Easy to
 * enable/disable Extent without code changes - Maintains Log4j logging
 * regardless of Extent status - Improves performance when Extent is disabled
 * 
 * Usage: Replace: ExtentCucumberAdapter.addTestStepLog("message"); With:
...
public class TestLogger {
	...
	/**
	 * Check if Extent reporting is enabled Caches the result to avoid repeated
	 * property lookups
	 * 
	...
	private static boolean isExtentEnabled() {
		if (extentEnabled == null) {
			try {
				...
				extentEnabled = configValue == null || !configValue.trim().equalsIgnoreCase("false");

				if (extentEnabled) {
					LOGGER.info(
							"... Extent Reporting is ENABLED - HTML reports will be generated with detailed test steps");
				} else {
	...
	/**
	 * Log a test step message - Always logs to Log4j - Conditionally logs to Extent
	...
	/**
	 * Log a test step message with custom Log4j logger Useful when you want to use
	 * a specific class's logger
	 * 
	...
	/**
	 * Log an error message - Always logs to Log4j as ERROR level - Conditionally
	 * logs to Extent Reports
	 * 
	...
	/**
	 * Log a warning message - Always logs to Log4j as WARN level - Conditionally
	 * logs to Extent Reports
	 * 
	...
	/**
	 * Add a screenshot to Extent Reports (if enabled) This method is specifically
	 * for Extent Reports screenshot integration
	 * 
	...
	/**
	 * Reset the cached configuration flag Useful for testing or when configuration
	 * changes at runtime
	 */
