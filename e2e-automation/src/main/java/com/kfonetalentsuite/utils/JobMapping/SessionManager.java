...
/**
 * SessionManager - Handles session validation, expiration detection, and automatic re-authentication
 * 
 * Features:
 * - Detects 401 Unauthorized errors and session expiration
 ...
 * 
 * Usage:
 *   SessionManager.validateSession(); // Check and refresh if needed
 *   SessionManager.executeWithSessionValidation(() -> { ... }); // Auto-retry with re-login
 */
public class SessionManager {
    ...
    public static boolean isSessionValid() {
        try {
            ...
            String sessionStorageCheck = (String) js.executeScript(
                "try {" +
                ...
                "  if (userRoles || authToken) return 'VALID';" +
                "  return 'INVALID';" +
     ...
     * Call this before critical operations (e.g., navigation, data submission)
     * 
    ...
    /**
     * Execute an operation with automatic session validation and re-login
     * If session expires during operation, automatically re-login and retry
     * 
    ...
    /**
     * Execute an operation with automatic session validation and re-login (with retry count)
     * 
     * @param operation The operation to execute
     * @param operationName Name for logging
     ...
     * @return Result of the operation
     */
    public static <T> T executeWithSessionValidation(java.util.function.Supplier<T> operation, String operationName, int maxRetries) {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                boolean isSessionError = errorMsg.contains("401") || 
                ...
                if (isSessionError && attempt < maxRetries) {
                    LOGGER.warn("⚠️ Session error detected in '{}': {} - Attempting re-login", operationName, e.getMessage());
