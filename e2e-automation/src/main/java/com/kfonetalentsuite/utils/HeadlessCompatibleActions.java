...
public class HeadlessCompatibleActions {
    ...
    /**
     * Scrolls to the bottom of the page in a headless-compatible way.
     * Uses document.documentElement.scrollHeight and triggers scroll events.
     * 
    ...
    /**
     * Scrolls to a specific element and brings it into view.
     * Works reliably in headless mode by using multiple strategies.
     * 
    ...
    /**
     * Clicks an element using headless-compatible strategies.
     * Tries multiple methods: regular click, JS click, and action click.
     ...
     * More reliable than isDisplayed() in headless mode.
     * 
    ...
    /**
     * Scrolls within a specific container element (for scrollable divs).
     * Useful for dropdowns, modals, etc.
     * 
     * @param container The container element with overflow
     * @param scrollAmount Amount to scroll in pixels (negative for up, positive for down)
     * @throws InterruptedException if thread sleep is interrupted
     */
    public void scrollWithinContainer(WebElement container, int scrollAmount) throws InterruptedException {
        try {
            js.executeScript("arguments[0].scrollTop = arguments[0].scrollTop + arguments[1];", container, scrollAmount);
            Thread.sleep(200);
    ...
    /**
     * Gets the current scroll height of the page.
     * Uses documentElement for headless compatibility.
     * 
    ...
    /**
     * Scrolls to the last visible element in a list.
     * Useful for triggering lazy loading.
     * 
    ...
    /**
     * Sends keys to an element with retry logic.
     * Clears the field first if specified.
     * 
     ...
     * Checks document.readyState and jQuery if available.
     * 
     * @param timeoutSeconds Timeout in seconds
     */
